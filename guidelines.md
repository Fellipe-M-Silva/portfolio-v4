...
- **Estética Geral:** O design deve ser simples, priorizando o conteúdo (os estudos de caso). Use bastante *whitespace* (respiro), tipografia legível e bordas sutis. Menos é mais. O foco está na arquitetura de informação.
...
- **Tipografia:** A fonte por enquanto será a Geist. Para detalhes como tags, chips e botões, penso em usar a Geist Mono.
...
- Para a versão de modo escuro, utilize *media query* para identificar o modo. O sistema deve suportar o tema do navegador/sistema, mas o usuário pode mudar entre sistema, claro e escuro
- Para a versão de alto contraste, utilize *media queries* como `@media (prefers-contrast: more)` para sobrescrever essas variáveis, garantindo proporções de contraste AAA.
- Nunca remova o *focus outline* (`outline: none`) sem fornecer uma alternativa visual clara (`:focus-visible`).
...
2.  **Exportação:** Use *named exports* (`export function NomeComponente`) em vez de *default exports*.
